...
if [ "$SENDER" = "aerospace_workspace_change" ]; then
  ...
  visible_workspaces=$(aerospace list-workspaces --all --format "%{workspace} %{workspace-is-visible}" | grep "true" | awk '{print $1}')
  
  # Update only visible workspaces
  for workspace in $visible_workspaces; do
    is_focused=$(aerospace list-workspaces --all --format "%{workspace} %{workspace-is-focused}" | grep "^$workspace " | awk '{print $2}')
